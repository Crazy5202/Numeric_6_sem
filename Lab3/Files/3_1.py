...
if __name__ == "__main__":
    ...
    """
    split = 100
    left_edge = solver.a[0]
    right_edge = solver.a[-1]
        
    x = [left_edge + (right_edge-left_edge)/split*i for i in range (split)]

    func_values = [solver.calc_eq(elem) for elem in x]
    plt.plot(x, func_values, label='Функция')

    values = [solver.calc_lagr(2, elem) for elem in x]
    plt.plot(x, values, label = 'Лагранж 2')
    ...
    values = [solver.calc_newton(2, elem) for elem in x]
    plt.plot(x, values, label = 'Ньютон 2')
